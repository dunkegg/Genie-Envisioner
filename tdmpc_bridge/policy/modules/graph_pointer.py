...
from tools.utils_graph import GCNEnhanceEncoder, GATEnhanceEncoder, EncoderMultiHeadAttention, DecoderMultiHeadAttention, PointerNet
...
class GraphPointerPolicy(nn.Module):
    ...
    def __init__(self, node_dim, edge_dim, embedding_dim, 
        ...
        self._using_relative_coords = using_relative_coords
        
    def forward(self, state):
        # Graph Encoder for Data Enhancement
        if self._encoder_type == 'GCN' or self._encoder_type == 'GAT':
            graphs_abs, graphs_rel, current_idx, action_idx, action_mask = state
            graphs = graphs_rel if self._using_relative_coords else graphs_abs
...
class GraphQNet(nn.Module):
    ...
    def __init__(self, node_dim, edge_dim, embedding_dim,
                 num_graph_padding, 
                 n_layer_encoder=4, n_layer_decoder=4,  
                 n_head=2, n_layer_with_edge_attr=4, 
                 encoder_type='GCN',
                 using_relative_coords=True):
