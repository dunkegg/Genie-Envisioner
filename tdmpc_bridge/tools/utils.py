...
def rescale_values(data, max_range=1):
    if len(data) == 1:
        return [max_range]
    min_v = min(data)
    max_v = max(data)
    output = []
    for value in data:
        normed_value = (value - min_v) / (max_v - min_v)
        # normed_value = max_range * (normed_value * 2 - 1)
        output.append(normed_value)
...
def CostMap(observation_list, 
            ...
            highlight=True, combine=True, batch=False, black_obs=True, hybrid_encoder_mode=False):
    '''
    input:
        numpy.ndarray : (B, T, N) or (B, T*N) or (T, N) or (T*N)
    output:
        如果 combine=True  : (B, 1, H, W) 或 (1, H, W)
        如果 combine=False : (B, T, H, W) 或 (T, H, W)

    新增参数:
        hybrid_encoder_mode:
            为新的 hybrid encoder 提供显式的时间维输入。
            当该参数为 True 时，会自动强制 combine=False，保留 8 帧时序地图，
            供 CNN stem + temporal attention 使用。
    '''
    if hybrid_encoder_mode:
        # hybrid encoder 需要保留完整时间维，不能把 8 帧提前 merge 成一张图
        combine = False

    # original_shape = observation_list.shape

    # # Check if input is (H, B, T*N)
    # if len(original_shape) == 3 and batch:
    #     H, batch_size, _ = original_shape
    #     observation_list = observation_list.reshape(H * batch_size, -1)  # (H*B, T*N)
    #     observation_list = observation_list.reshape(H * batch_size, -1, obs_dim)  # (H*B, T, N)
    # else:
    #     if not batch:
    #         observation_list = np.expand_dims(observation_list, axis=0)  # (1, T, N) or (1, T*N)
    ...
    return dynamic_ego_obs.astype(np.float32)

def CostMapDual(observation_list,
                laser_dim, laser_range, img_h, img_w, obs_dim, highlight_iterations,
                batch=False, black_obs=True):
    """
    为 dual-branch encoder 同时生成：
    1) combine=True  的灰度融合图
    2) combine=False 的完整时序图
    ...
    return combine_map, temporal_map
...
def merge_visualization(traj_img, lidar_ego_imgs,
    ...
    # print("bound_height:", bound_height)
    # print("img_h. img_w: ", img_h, img_w)
    # # print("bar: ", bar)
    # print("range: ", traj_img_range)
...
def system_info(init_process_memory, init_free_memory, if_bar=False):
    ...
    return system_memory_percent, cpu_usage_percent, gpu_usage_percent
...
class CoarseSimulator():
    ...
    def predict_mpc_traj(self, mpc_v, mpc_w, x, y, th, dt, type, sim_plat='gazebo', rx=0, ry=0, rth=0):
        next_xs = []
        next_ys = []
        next_ths = []
        if sim_plat == 'irsim':
        ...
        if type == 'mppi':
            for i in range(mpc_v.shape[0]):
                x = - mpc_v[i, :] * np.cos(th) * dt  * self.pixel_per_meter + x
                y = - mpc_v[i, :] * np.sin(th) * dt  * self.pixel_per_meter + y
                th = mpc_w[i, :] * dt + th
                ...
                if sim_plat == 'irsim':
                    rx = rx + mpc_v[i, :] * np.cos(rth) * dt
                    ry = ry + mpc_v[i, :] * np.sin(rth) * dt
                    rth += mpc_w[i, :] * dt
                    next_xs_irsim.append(rx)
                    next_ys_irsim.append(ry)
                    next_ths_irsim.append(rth)

            if sim_plat == 'irsim':
                # Stack to shape: [6, 512]
                xs = np.stack(next_xs_irsim, axis=0)  # [6, 512]
                ys = np.stack(next_ys_irsim, axis=0)  # [6, 512]
                ths = np.stack(next_ths_irsim, axis=0)  # [6, 512]

                # Transpose to [512, 6] then zip to form list of [x,y,theta] per 512 trajs
                traj_irsim = [np.array([xs[:, i], ys[:, i], ths[:, i]]) for i in range(xs.shape[1])]
                return np.array(next_xs), np.array(next_ys), np.array(next_ths), traj_irsim  # List of 512 elements, each [3, 6] array
            ...
            if sim_plat == 'irsim':
                traj_irsim = np.array([next_xs_irsim, next_ys_irsim, next_ths_irsim])
                return np.array(next_xs), np.array(next_ys), np.array(next_ths), traj_irsim
            
        return np.array(next_xs), np.array(next_ys), np.array(next_ths)
...
class TrajImage():
    def __init__(self, img_range, scale, idx, box_list, circle_list, name="", world_type="state_sep"):
