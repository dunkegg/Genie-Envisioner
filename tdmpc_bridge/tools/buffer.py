...
from tools.utils_graph import padding_list, to_adjacency_matrix
...
class ReplayBuffer_Graph(object):
    ...
    def add(self, state, action, next_state, reward, done, num_set=0):
        ...
        if self._using_pyg:               
            data = (dict(pyg_graph_abs=state['pyg_graph_abs'], pyg_graph_rel=state['pyg_graph_rel'], current_idx=state['current_idx'], action_idxes=state['action_idxes'], action_mask=state['action_mask']),
                    torch.tensor(action), 
                    dict(pyg_graph_abs=next_state['pyg_graph_abs'], pyg_graph_rel=next_state['pyg_graph_rel'], current_idx=next_state['current_idx'], action_idxes=next_state['action_idxes'], action_mask=next_state['action_mask']),
                    reward, 
                    1. - done)
        else:
    ...
    def sample(self, batch_size, num_set=0):
        ...
        if self._using_pyg:
            ...
            batch_next_graph_abs = []
            batch_next_graph_rel = []
            ...
            for i, idx in enumerate(idxes):
                ...
                batch_graph_abs.append(state['pyg_graph_abs'])
                batch_graph_rel.append(state['pyg_graph_rel'])
                ...
                batch_next_graph_abs.append(next_state['pyg_graph_abs'])
                batch_next_graph_rel.append(next_state['pyg_graph_rel'])
            ...
            return (dict(pyg_graph_abs=Batch.from_data_list(batch_graph_abs), pyg_graph_rel=Batch.from_data_list(batch_graph_rel), current_idx=batch_current_idx.long(), action_idxes=batch_action_idxes.long(), action_mask=batch_action_mask),
                    batch_action.long(), 
                    dict(pyg_graph_abs=Batch.from_data_list(batch_next_graph_abs), pyg_graph_rel=Batch.from_data_list(batch_next_graph_rel), current_idx=batch_next_current_idx.long(), action_idxes=batch_next_action_idxes.long(), action_mask=batch_next_action_mask),
